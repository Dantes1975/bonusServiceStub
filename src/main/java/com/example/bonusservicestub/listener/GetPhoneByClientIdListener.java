...
import com.example.bonusservicestub.service.phone.GetPhoneByClientIdSender;
...
@Slf4j
public class GetPhoneByClientIdListener implements MessageListener {

    private final ObjectMapper objectMapper = new ObjectMapper();

    @Autowired
    @Getter(PRIVATE) private GetPhoneByClientIdSender sender;
    ...
    public void onMessage(Message message) {
        try {
            if (message instanceof TextMessage) {
                ...
                AdditionPhoneGetDetailsRequest request = objectMapper.readValue(objectMessage, AdditionPhoneGetDetailsRequest.class);
                sender.sendGetPhoneByClientIdResponse(request, jmsCorrelationID, timeToLive, transactionID);
            }
        } catch (Exception e) {
